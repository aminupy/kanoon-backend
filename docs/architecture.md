...
The S3 adapter uses two endpoint views with the same credentials, bucket, and region. Backend-only
operations (health, bucket management, object inspection, and deletion) use
`KANOON_S3_ENDPOINT_URL`, which may resolve only on the service network. Presigned POST and GET URLs
are generated by a second client configured with `KANOON_S3_PUBLIC_ENDPOINT_URL`, so the browser
receives a reachable host and that host is included in the original SigV4 signature. Signed URLs are
never generated against the internal host and rewritten afterward.
